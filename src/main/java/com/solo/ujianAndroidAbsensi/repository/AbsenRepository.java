...
public interface AbsenRepository extends JpaRepository<Absen, Long>{
	...
	@Query(value = "Select * FROM absen WHERE username = ?1 AND tanggal_masuk LIKE %?2%", nativeQuery = true)
	public Absen getByUsernameAndDateIn(String username, String date);
	
	@Query(value = "Select * FROM absen WHERE username = ?1 AND tanggal_keluar LIKE %?2%", nativeQuery = true)
